...
like($req_2->keyinfo(), qr/47:b0:60:58:46:3e:68:46/, "Read Key Info from CSR.csr");
like($req_2->pubkey_type(), qr/rsa/, "Read public key type from CSR.csr");
like($req_2->get_pem_pubkey(), qr/cIPeHtJryFNmaGZJWNZW7AvlriVtKigGEFtT9G7PqSw2h8/, "Read PEM public key from CSR.csr");
ok($req_2);
}
...
like($req_csr->keyinfo(), qr/47:b0:60:58:46:3e:68:46/, "Read Key Info from CSR.csr (PEM)");
like($req_csr->pubkey_type(), qr/rsa/, "Read public key type from CSR.csr (PEM)");
like($req_csr->get_pem_pubkey(), qr/cIPeHtJryFNmaGZJWNZW7AvlriVtKigGEFtT9G7PqSw2h8/, "Read PEM public key from CSR.csr (PEM)");
ok($req_csr);
}
...
like($req_der->keyinfo(), qr/47:b0:60:58:46:3e:68:46/, "Read Key Info from CSR.der (DER)");
like($req_der->pubkey_type(), qr/rsa/, "Read public key type from CSR.der (DER)");
like($req_der->get_pem_pubkey(), qr/cIPeHtJryFNmaGZJWNZW7AvlriVtKigGEFtT9G7PqSw2h8/, "Read PEM public key from CSR.der (DER)");
ok($req_der);
}
